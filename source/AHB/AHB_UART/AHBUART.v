	  ...
	  begin
		if(HREADY)
			begin
			  ...
			  last_HADDR <= HADDR;
			  HREADYOUT <= 1'b0;
//			  uart_wr <=  HTRANS[1] & HWRITE & HSEL& (HADDR[31:29]==3'b011);
			end
		else
			begin
			  last_HTRANS <= 2'b0;
...
//	  end
  ...
  //UART  write select
  assign uart_wr = last_HTRANS[1] & last_HWRITE & last_HSEL& (last_HADDR[31:29]==3'b011);
  //Only write last 8 bits of Data
  assign uart_wdata = HWDATA[7:0];

  //UART read select
  assign uart_rd = last_HTRANS[1] & ~last_HWRITE & last_HSEL & (last_HADDR[31:29]==3'b011) & (last_HADDR[2:0]==3'b100);
  

  assign HRDATA = (last_HADDR[31:29]==3'b011) ? ((last_HADDR[2:0]==3'b000)?{24'h0000_00,status}:{24'h0000_00,uart_rdata}):32'b0;
